...
<script>
  ...
  export default {
    ...
    mounted(){
      this.axios.get('https://api.ostep.com.cn/bzlq/candidate/result?js_code=' + getQuery('code')).then(res => {
    ...
    methods: {
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.axios.get(`https://api.ostep.com.cn/bzlq/candidate/result/byIdNumAndName?id_num=${query.identity_num}&name=${query.name}
`).then(res => {
              if (res.data.result_code === 200) {
                this.openType = true
                this.fetchData = res.data.data
